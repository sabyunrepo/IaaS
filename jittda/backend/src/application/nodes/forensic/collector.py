...
GitHub GraphQL으로 후보자 리포 목록 → Funnel Stage 1-3 필터링 → shallow clone + sparse checkout.
"""
...
async def collector_worker(state: ForensicState) -> dict[str, Any]:
    ...
    Phase 8: repo_collector_node가 MetaGraph 레벨에서 이미 clone을 완료한 경우,
    collected_repos/repo_local_paths가 주입되어 있으므로 skip한다.
    """
    # Phase 8: pre-loaded data가 있으면 skip (repo_collector_node에서 이미 처리됨)
    if state.get("collected_repos") and state.get("repo_local_paths"):
    ...
    for repo in top_repos:
        try:
            ...
            # JD 언어 기반 sparse checkout 적용 (해당 확장자 파일만 체크아웃)
            if sparse_patterns:
                try:
                    await clone_manager.sparse_checkout(path, sparse_patterns)
                    logger.info("Sparse checkout applied: %s patterns for %s", len(sparse_patterns), repo.name)
                except Exception:
                    logger.warning("Sparse checkout failed for %s, using full checkout", repo.name)
